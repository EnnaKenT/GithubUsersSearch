...
class UsersViewModel(private val usersByLoginUseCase: GetUsersByLoginUseCase) : BaseViewModel() {
    ...
    fun searchQueryChanged(text: String) = requestFirstPageUsers(text,
        QUERY_DELAY
    )
    ...
    private suspend fun requestUsers() {
        if (searchLogin.isEmpty()) {
            ...
            val users = usersByLoginUseCase(GetUsersByLoginUseCase.Params.withParams(searchLogin, page))
